...
type FlunderLister interface {
	...
	List(selector labels.Selector) (ret []*custom.Flunder, err error)
...
// List lists all Flunders in the indexer.
func (s *flunderLister) List(selector labels.Selector) (ret []*custom.Flunder, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*custom.Flunder))
	})
	return ret, err
}
...
type FlunderNamespaceLister interface {
	...
	Get(name string) (*custom.Flunder, error)
...
// List lists all Flunders in the indexer for a given namespace.
func (s flunderNamespaceLister) List(selector labels.Selector) (ret []*custom.Flunder, err error) {
...
// Get retrieves the Flunder from the indexer for a given namespace and name.
func (s flunderNamespaceLister) Get(name string) (*custom.Flunder, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(custom.Resource("flunder"), name)
	}
	return obj.(*custom.Flunder), nil
}
