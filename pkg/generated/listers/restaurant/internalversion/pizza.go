...
type PizzaLister interface {
	...
	List(selector labels.Selector) (ret []*restaurant.Pizza, err error)
...
// List lists all Pizzas in the indexer.
func (s *pizzaLister) List(selector labels.Selector) (ret []*restaurant.Pizza, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*restaurant.Pizza))
	})
	return ret, err
}
...
type PizzaNamespaceLister interface {
	...
	Get(name string) (*restaurant.Pizza, error)
...
// List lists all Pizzas in the indexer for a given namespace.
func (s pizzaNamespaceLister) List(selector labels.Selector) (ret []*restaurant.Pizza, err error) {
...
// Get retrieves the Pizza from the indexer for a given namespace and name.
func (s pizzaNamespaceLister) Get(name string) (*restaurant.Pizza, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(restaurant.Resource("pizza"), name)
	}
	return obj.(*restaurant.Pizza), nil
}
